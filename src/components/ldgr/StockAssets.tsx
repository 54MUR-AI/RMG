/**
 * StockAssets — Manage stocks, ETFs, and mutual funds in LDGR Assets.
 */

import { useState, useEffect } from 'react'
import { Plus, Trash2, Edit2, RefreshCw, TrendingUp, TrendingDown, BarChart3 } from 'lucide-react'
import { useAuth } from '../../contexts/AuthContext'
import {
  getAssetsByType, addAsset, updateAsset, deleteAsset, enrichAssetsWithPrices,
  EQUITY_TYPES, ASSET_TYPE_CONFIG,
  ...
  if (loading) {
    return (
      <div className="text-center py-12">
        ...
        <p className="mt-4 text-white/70">Loading equities...</p>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black text-white flex items-center gap-2">
            <BarChart3 className="w-6 h-6 text-samurai-red" />
            Stocks, ETFs & Mutual Funds
          </h2>
          <p className="text-white/70 text-sm mt-1 hidden sm:block">
            Track your equity holdings with live pricing
          </p>
        </div>
      ...
      {assets.length > 0 && totalValue > 0 && (
        <div className="bg-samurai-grey-darker border-2 border-samurai-grey rounded-lg p-4 flex items-center justify-between">
          <div>
            <p className="text-white/60 text-xs">Total Equity Value</p>
      ...
      {assets.length === 0 ? (
        <div className="text-center py-12 bg-samurai-grey-darker rounded-lg border-2 border-samurai-grey">
          ...
          <p className="text-white/70 text-lg">No equities yet</p>
          <p className="text-white/50 text-sm mt-2">Add stocks, ETFs, or mutual funds to track</p>
        </div>
      ) : (
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm text-white/60">
                        {a.symbol && <span className="font-mono text-samurai-red">{a.symbol}</span>}
                        {a.symbol && ' · '}{a.quantity} shares @ ${a.cost_basis.toFixed(2)}
                      </p>
                    </div>
                  </div>
...
function StockModal({ existing, onSave, onClose }: {
  existing: LdgrAsset | null
  onSave: (input: LdgrAssetInput) => Promise<void>
  onClose: () => void
}) {
  const [name, setName] = useState(existing?.asset_name || '')
  const [type, setType] = useState<AssetType>(existing?.asset_type as AssetType || 'stock')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name || !symbol || isNaN(qty) || qty <= 0 || isNaN(cost) || cost < 0) return
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <select value={type} onChange={e => setType(e.target.value as AssetType)}
              ...
              <option value="mutf">🏦 Mutual Fund</option>
            </select>
          </div>
          <div>
            <label className="block text-white font-semibold mb-2">Ticker Symbol</label>
            <input type="text" value={symbol} onChange={e => setSymbol(e.target.value)}
              placeholder="AAPL, VOO, VTSAX"
              className="w-full px-4 py-3 bg-samurai-black border-2 border-samurai-grey rounded-lg text-white placeholder-white/50 focus:border-samurai-red focus:outline-none font-mono" required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-white font-semibold mb-2">Cost/Share ($)</label>
          ...
          <div className="flex gap-3 pt-4">
            <button type="submit" disabled={saving}
              ...
              {saving ? 'Saving...' : existing ? 'Update' : 'Add Position'}
            </button>
